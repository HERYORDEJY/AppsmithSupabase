...
export async function sync() {
  const username = await getWatermelonDb().localStorage.get<string>('username');
  if (!username) {
  ...
  await synchronize({
    ...
    pullChanges: async ({lastPulledAt}) => {
      ...
      const {data, error} = await supabaseClient.rpc('pull', {
    ...
    pushChanges: async ({changes, lastPulledAt}) => {
      ...
      const {error} = await supabaseClient.rpc('push', {
        changes,
        p_record_owner: username,
      });
